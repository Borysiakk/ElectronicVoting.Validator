using ElectronicVoting.Validator.Domain.Entities.Election;
using Microsoft.EntityFrameworkCore;

namespace ElectronicVoting.Validator.Infrastructure.EntityFramework.Election.Seeders;

public static class ValidatorNodeSeeder
{
    public static async Task SeedAsync(DbContext context, CancellationToken cancellationToken = default)
    {
        if (!await context.Set<ValidatorNode>().AnyAsync(cancellationToken)) 
            return;
        
        var validatorNodes = CreateValidatorNodes();
        context.Set<ValidatorNode>().AddRange(validatorNodes);
        await context.SaveChangesAsync(cancellationToken);
    }
    
    private static ValidatorNode[] CreateValidatorNodes()
    {
        return new[]
        {
            new ValidatorNode
            {
                Id = Guid.NewGuid(),
                IsLeader = true,
                Name = "electronicvoting.validator_a",
                ServerUrl = "http://electronicvoting.validator_a:8080",
                PublicKey = "-----BEGIN CERTIFICATE-----\\n-----END CERTIFICATE-----\n"
            },
            new ValidatorNode
            {
                Id = Guid.NewGuid(),
                Name = "electronicvoting.validator_b",
                ServerUrl = "http://electronicvoting.validator_b:8080",
                PublicKey = "-----BEGIN CERTIFICATE-----\\n-----END CERTIFICATE-----\n"
            }
        };
    }

}